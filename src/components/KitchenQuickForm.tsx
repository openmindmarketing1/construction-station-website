...
export default function KitchenQuickForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (typeof window !== "undefined" && window.fbq) {
        window.fbq("track", "Lead", { content_name: "kitchen_contact" });
      }
      setSubmitted(true);
    } catch {
    ...
    return (
      <div className="bg-white p-8 md:p-10 border-t-4 border-gold text-center">
        ...
        <p className="text-navy/65 text-sm">
          Thanks {form.name.split(" ")[0]}. A team member is already reviewing your request.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-8 md:p-10 border-t-4 border-gold">
      <div className="text-gold text-xs tracking-[0.3em] uppercase mb-2">Get a Callback</div>
      <h3 className="font-display text-navy text-2xl md:text-3xl mb-1">
        Prefer us to call you?
      </h3>
      <p className="text-navy/60 text-sm mb-6">
        Leave your info and we&rsquo;ll reach out within 2 business hours.
      </p>
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          <label htmlFor="kf-name" className="block text-xs uppercase tracking-wider text-navy/70 mb-2">
        ...
        <div>
          <label htmlFor="kf-phone" className="block text-xs uppercase tracking-wider text-navy/70 mb-2">
        ...
        <div>
          <label htmlFor="kf-email" className="block text-xs uppercase tracking-wider text-navy/70 mb-2">
            Email
          </label>
          <input
            id="kf-email"
